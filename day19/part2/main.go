...
func process(data [][]aoc.Vector3, common int) int {
	...
	var scanners []aoc.Vector3

	for len(otherScanners) > 0 {
		var found []int

		for id, s := range otherScanners {
			if scanner := mergeScanner(baseScanner, s, common); scanner != nil {
				fmt.Printf("Found: %v\n", id)
	...
	for i := 0; i < len(scanners)-1; i++ {
		...
		for j := i + 1; j < len(scanners); j++ {
			v1 := scanners[j]
			s := v0.Sub(v1)
			max = aoc.Max(max, s.ManhattanLength())
		}
	}
...
func mergeScanner(baseScanner map[aoc.Vector3]bool, scanner scanner, common int) (scannerPosition *aoc.Vector3) {
	for _, v := range scanner.variants {
		if sp := mergeVariant(baseScanner, v, common); sp != nil {
			return sp
		}
	}
...
func mergeVariant(baseScanner map[aoc.Vector3]bool, variant variant, common int) (scanner *aoc.Vector3) {
